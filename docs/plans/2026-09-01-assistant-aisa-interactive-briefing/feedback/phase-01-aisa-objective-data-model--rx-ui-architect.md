...
|---|---|---|---|
| 1 | `.claude/agents/rx-ui-architect.md` | Agregar `src/data.ts` al Alcance del architect, acotado a "seed rows que acompañan un cambio de shape en `src/types.ts`" — hoy el agent file no lo lista pero el orquestador se lo asigna. | deferred to /improve — clarificación de scope, sin defecto asociado |
| 2 | `.claude/orchestrators/ui.md` | En las reglas de precisión del brief: cuando una feature agrega entradas a una tabla de matching por keyword/substring, el brief DEBE incluir un paso de verificación de colisión (grep de cada keyword nueva contra los prompts, respuestas simuladas y datos demo interpolados existentes). | → applied 0ac9364 (regla agregada a `docs/plans/_templates/plan.md` § BRIEF ACCURACY en vez de `orchestrators/ui.md` — mismo efecto, es donde ya viven las otras dos reglas de precisión de brief) |
| 3 | `CLAUDE.md` (raíz) | Documentar el invariante de `src/services.ts`: `combineDebriefText` pliega el prompt de cada pregunta respondida en el texto evaluado por `evaluateVisitObjectives`, así que prompts, respuestas simuladas y strings de datos interpolados comparten un namespace global de keywords. Hoy solo vive en un doc comment de `services.ts`. | → applied 0ac9364 |
