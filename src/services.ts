...
export const buildBriefingFollowUpAnswer = (
  question: string,
  ...
  const topics = briefing.recentTopics.length ? briefing.recentTopics : [];
  const blockers = briefing.blockers.length ? briefing.blockers : [];

  if (asked.includes('balance') || asked.includes('billing') || asked.includes('invoice') || asked.includes('payment')) {
    return account.creditHold
      ? `Finance flags ${formatCurrency(account.creditHold.amount)} outstanding at ${account.creditHold.overdueDays} days late. Clear the path to payment before you commit to new scope.`
      : `No billing hold on ${account.name}. Treat commercial terms as open and focus on the next commitment.`;
  }

  if (asked.includes('hearing') || asked.includes('signal') || asked.includes('news') || asked.includes('outside')) {
    const signals = account.externalSignals ?? [];
    return signals.length
      ? `Outside the CRM I picked up ${signals.length} item(s). Start with ${signals[0].source}: ${signals[0].summary}. Confirm it in their words before you act on it.`
      : `Nothing outside the CRM on ${account.name} right now. Everything below comes from your own account history.`;
  }

  // Deliberately narrow: a bare `follow-up` probe would also swallow the "what changed since
  // the last ${account.status} conversation?" question for accounts whose status is
  // "Needs follow-up", collapsing two suggested questions onto one answer.
  if (asked.includes('next action') || asked.includes('follow-up should be created') || asked.includes('after this visit')) {
    return account.nextAction
      ? `The agreed next action is: ${account.nextAction}. Confirm the owner and the date, then capture it in the debrief so the CRM matches the room.`
      : `Nothing is committed yet. Leave with one named owner and one date, otherwise this visit will not move the account.`;
  }

  if (asked.includes('risk') || asked.includes('blocker') || asked.includes('concern') || asked.includes('escalation')) {
    return blockers.length
      ? `Risk level is ${briefing.riskLevel}. The live blockers are: ${blockers.join('; ')}. Ask which one they would remove first.`
      : `Risk level is ${briefing.riskLevel} with no open blockers logged. Use the time to widen the relationship instead of firefighting.`;
  }

  if (asked.includes('changed') || asked.includes('since') || asked.includes('last') || asked.includes('recent')) {
    return topics.length
      ? `The last threads on record are: ${topics.join(' | ')}. Open with one of these so they hear continuity, then ask what moved since.`
      : `Nothing recent is logged on ${account.name}. Treat this as a reset conversation and re-establish current priorities.`;
  }

  // Matches the opportunity question generated by `buildPreMeetingBriefing`, in both of its
  // forms: `Is ${opportunity.name} still in ${opportunity.stage}?` and, with no opportunity,
  // `Is there a new commercial opportunity to capture?`. The stage/name probes are last on
  // purpose — a real stage word ("Pricing", "Evaluation") also shows up inside the other two
  // generated questions via `account.nextAction` / `account.status`, so the more specific
  // branches above must claim those first.
  const matchesOpportunityQuestion = asked.includes('still in')
    || asked.includes('commercial opportunity')
    || asked.includes('pipeline')
    || (opportunity !== undefined
      && (asked.includes(opportunity.name.toLowerCase()) || asked.includes(opportunity.stage.toLowerCase())));

  if (matchesOpportunityQuestion) {
    return opportunity
      ? `${briefing.opportunitySummary ?? `${opportunity.name} is in ${opportunity.stage}.`} Next step on record is "${opportunity.nextStep}" — push for the date it lands, not just a stage confirmation.`
      : `There is no open opportunity on ${account.name}. Listen for a trigger you can turn into one, then log it before you leave.`;
  }

  return `${briefing.executiveSummary} Risk level is ${briefing.riskLevel}${blockers.length ? `, with ${blockers.length} open blocker(s)` : ''}. Bring that context into the answer you are looking for.`;
};
